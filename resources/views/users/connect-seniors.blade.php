@foreach ($seniorsExchanges as $index => $exchangeDetail)
    <div class="col-md-6">
            ...
            <div class="peers-area">
                ...
                <div>
                    ...
                    <div class="user-info-area">
                        <ul>
                            <li><u>{!! trans('community.return_overseas') !!}</u></li>
                            ...
                            @if ($exchangeDetail['term_from'] && $exchangeDetail['term_to'])
                                <li>{!! trans('community.school_term') !!}: {{ Carbon\Carbon::parse($exchangeDetail['term_from'])->format('M Y') }} - {{ Carbon\Carbon::parse($exchangeDetail['term_to'])->format('M Y') }}</li>
                            @else
                                <li>{!! trans('community.school_term') !!}: {{$exchangeDetail['exchangeTerm']}}</li>
                            @endif
                        </ul>
                        @if (Auth::guest())
                            <p class="pro-button"><a href="javascript:void(0);" onclick="showError('{!! trans('community.please_login') !!}', 10000);$('html, body').animate({scrollTop : 0}, 800);">{!! trans('community.seek_advice') !!}</a></p>
                        @elseif (Auth::user()->id == $exchangeDetail['userId'])
                            <p class="pro-button"><a href="javascript:void(0);" onclick="return loggedinUser();">{!! trans('community.seek_advice') !!}</a></p>
                        @else
                            <p class="pro-button"><a href="javascript:void(0);" onclick="openModalPop(this);" data-type="2" data-userid="{{Auth::user()->id}}" data-toid="{{$exchangeDetail['userId']}}" data-personalizedMsg="{{ str_replace('{host_institution}', $exchangeDetail['hostUniversity'], str_replace('{name}', Auth::user()->fname.' '.Auth::user()->lname, str_replace('{exchange_name}', $exchangeDetail['fname'].' '.$exchangeDetail['lname'], trans('community.peer_start_chat')))) }}">{!! trans('community.seek_advice') !!}</a></p>
                        @endif
                    </div>
                </div>
            </div>
